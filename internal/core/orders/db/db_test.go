...
func TestOrdersRepository_Add_OK(t *testing.T) {
	...
	before := time.Now()
	repo := odb.New(pgpool)
	o, err := repo.Add(context.TODO(), models.NewOrder("1234567812345670", u.ID), orders.AddNoop)
	require.NoError(t, err)
...
func TestOrdersRepository_Add_ErrorOnInvalidStatus(t *testing.T) {
	...
	repo := odb.New(pgpool)
	newOrder := models.NewOrder("1234567812345670", u.ID)
	newOrder.Status = "foo"
...
func TestRepository_GetListForUser_OK(t *testing.T) {
	...
	for _, number := range []string{"1234567812345670", "4561261212345467", "49927398716"} {
		_, err = repo.Add(context.TODO(), models.NewOrder(number, u.ID), orders.AddNoop)
		require.NoError(t, err)
	}
...
func TestRepository_UpdateStatus_OK(t *testing.T) {
	...
	repo := odb.New(pgpool)
	o, _ := repo.Add(context.TODO(), models.NewOrder("1234567812345670", u.ID), orders.AddNoop)
	other, _ := repo.Add(context.TODO(), models.NewOrder("4561261212345467", u.ID), orders.AddNoop)

	err = repo.UpdateStatus(context.TODO(), o.ID, models.OrderStatusProcessed, 100.5)
	require.NoError(t, err)

	o2, _ := repo.GetByNumber(context.TODO(), "1234567812345670")
	assert.Equal(t, models.OrderStatusProcessed, o2.Status)
	assert.Equal(t, 100.5, o2.Accrual)
	...
	assert.Equal(t, 0.0, other2.Accrual)
}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr {
				...
				assert.Equal(t, tt.accrual, o2.Accrual)
			}
		})
	}
}
